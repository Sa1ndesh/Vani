...
(function () {
  ...
  window.ArogyaBot = {
    ...
    async send(text) {
      ...
      const apiBaseUrl = window.API_BASE_URL || ((window.location.protocol === "file:" || window.location.origin === "null" || ["localhost", "127.0.0.1", "0.0.0.0"].includes(window.location.hostname)) ? "http://localhost:5000/api" : `${window.location.origin}/api`);
    ...
    async clearChat() {
      const apiBaseUrl = window.API_BASE_URL || ((window.location.protocol === "file:" || window.location.origin === "null" || ["localhost", "127.0.0.1", "0.0.0.0"].includes(window.location.hostname)) ? "http://localhost:5000/api" : `${window.location.origin}/api`);
      try { await fetch(`${apiBaseUrl}/chat/${SESSION_ID}`, { method: "DELETE" }); } catch (_) {}
